...
const svg = readFileSync('C:/Users/david/Vault/Business/Startup/kadabra/brand/kadabra-second-edition.svg', 'utf8');
...
let sp = readFileSync('C:/Users/david/Vault/Business/Startup/kadabra/site/sprites.svg', 'utf8');
sp = sp.replace(/<symbol id="logo-full"[\s\S]*?<\/symbol>/, full).replace(/<symbol id="logo-mark"[\s\S]*?<\/symbol>/, mark);
writeFileSync('C:/Users/david/Vault/Business/Startup/kadabra/site/sprites.svg', sp);
console.log('full', vbFull, 'mark', vbMark, 'paths', r.paths.length, r.paths.map(x=>x.id+'@'+x.parent).join(' '));
writeFileSync('C:/Users/david/Vault/Business/Startup/kadabra/site/favicon.svg',
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${vbMark}"><path d="${m.d}" fill="#F3EEE4" fill-rule="${fr(m.style)}"/></svg>`);
