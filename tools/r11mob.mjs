...
await p.screenshot({ path: `shots/r11/${tag}-0.png` });
for (let i = 0; i < 5; i++) {
  await p.click(i % 2 ? '#bLeft' : '#bRight'); await sleep(250);
  if (i === 1) await p.screenshot({ path: `shots/r11/${tag}-mid.png` });
  await sleep(700);
  if (i === 2) await p.screenshot({ path: `shots/r11/${tag}-1.png` });
}
await sleep(600); await p.screenshot({ path: `shots/r11/${tag}-veil.png` });
await sleep(1500); await p.screenshot({ path: `shots/r11/${tag}-veil2.png` }); await sleep(3000);
await p.evaluate(() => document.getElementById('desk').scrollIntoView()); await sleep(900);
await p.screenshot({ path: `shots/r11/${tag}-desk.png` });
await p.evaluate(() => scrollBy(0, 700)); await sleep(900);
await p.screenshot({ path: `shots/r11/${tag}-desk2.png` });
